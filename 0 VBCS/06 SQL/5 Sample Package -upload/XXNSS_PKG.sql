...
CREATE OR REPLACE PACKAGE BODY xxnss_pkg AS
    ...
    BEGIN
        l_status := 'NA';
        OPEN nss_data_list(p_batch_no);
        LOOP
            FETCH nss_data_list INTO
                l_school_name,
                l_s_no;
            EXIT WHEN nss_data_list%notfound;
            SELECT
                COUNT(*)
            INTO l_cnt
            FROM
                xxlookup_details
            WHERE
                upper(lookup_values) = upper(l_school_name)
                and LOOKUP_CATERGORY_CODE = 'SCHOOL';

            IF l_cnt = 0 THEN
                INSERT INTO xxnss_validation_error_tbl VALUES (
                    xxnss_validation_error_s.NEXTVAL,
                    p_batch_no,
                    l_s_no,
                    'Wrong school name found.'
                );
        ...
        COMMIT;
        p_status := l_status;
    END xxnss_school_validate_nss_data;
    ...
    BEGIN
        ...
        LOOP
            ...
            FROM
                xxlookup_details
            WHERE
                upper(lookup_values) = upper(l_brand_name)
                 and LOOKUP_CATERGORY_CODE = 'BRAND';
        ...
        COMMIT;
        p_status := l_status;
    END xxnss_brand_validate_nss_data;
    ...
    BEGIN
        ...
        LOOP
            ...
            FROM
                xxlookup_details
            WHERE
                upper(lookup_values) = upper(l_board_name);
        ...
        COMMIT;
        p_status := l_status;
    END xxnss_board_validate_nss_data;
    ...
    BEGIN
        ...
        LOOP
            ...
            FROM
                xxlookup_details
            WHERE
                upper(lookup_values) = upper(l_grade_name);
        ...
        COMMIT;
        p_status := l_status;
    END xxnss_grade_validate_nss_data;
    ...
    BEGIN
        ...
        COMMIT;
        p_status := l_status;
    END xxnss_category_validate_data;
    ...
    BEGIN
        ...
        COMMIT;
        p_status := l_status;
    END xxnss_terms_phase_validate_nss_data;
    ...
    PROCEDURE xxnss_update_nss_details_batch_proc (
        ...
        l_nss_arr         json_array_t;
        l_nss_obj         json_object_t;
        p_last_updated_by VARCHAR2(500);
    BEGIN
        l_top_obj := json_object_t(p_data);
        p_last_updated_by := l_top_obj.get_string('p_last_updated_by');
        l_nss_arr := l_top_obj.get_array('nss_items');
        FOR i IN 0..l_nss_arr.get_size - 1 LOOP
            l_nss_obj := TREAT(l_nss_arr.get(i) AS json_object_t);
            l_nss_details_tab.extend;
            l_nss_details_tab(l_nss_details_tab.last).nss_count := l_nss_obj.get_string('nss_count');
            l_nss_details_tab(l_nss_details_tab.last).lc_count := l_nss_obj.get_string('lc_count');
            l_nss_details_tab(l_nss_details_tab.last).nss_detail_id := l_nss_obj.get_string('nss_detail_id');
            l_nss_details_tab(l_nss_details_tab.last).last_updated_by := p_last_updated_by;
            l_nss_details_tab(l_nss_details_tab.last).last_updated_date := sysdate;
        END LOOP;

        FOR j IN 1..l_nss_details_tab.count LOOP
            UPDATE xxnss_details
            SET
                nss_count = l_nss_details_tab(j).nss_count,
                lc_count = l_nss_details_tab(j).lc_count,
                last_updated_by = l_nss_details_tab(j).last_updated_by,
                last_updated_date = l_nss_details_tab(j).last_updated_date
            WHERE
                nss_detail_id = l_nss_details_tab(j).nss_detail_id;

        END LOOP;

        COMMIT;
        x_status := 'Updated Successfully';
    EXCEPTION
        WHEN OTHERS THEN
            x_status := sqlerrm || dbms_utility.format_error_backtrace;
            ROLLBACK;
    END xxnss_update_nss_details_batch_proc;
    ...
    BEGIN
        l_status := 'Y';
        INSERT INTO xxnss_approval_action_history VALUES (
            xxnss_approval_action_history_s.NEXTVAL,
            p_nss_id,
            p_curr_seq,
            'Pending for Approval',
            '',
        ...
        IF p_curr_seq = 1 THEN
            UPDATE xxnss_approvals_info
            SET
                approval_status = 'Approval pending with Level 1'
            WHERE
                approvals_info_id = p_nss_id;

        ELSE
            UPDATE xxnss_approvals_info
            SET
                approval_status = 'Approval pending with Level '
                                  || p_curr_seq
                                  || 'and Approved by Level '
                                  || ( p_curr_seq - 1 )
            WHERE
                approvals_info_id = p_nss_id;
        ...
        p_status := l_status;
        COMMIT;
    END xxnss_initiate_approval_flow;
    ...
    BEGIN
        l_status := 'Y';
        UPDATE xxnss_approval_action_history
        SET
            approval_status = p_approver_response
        WHERE
            approvals_info_id = p_nss_id;

        p_status := l_status;
        COMMIT;
    END xxnss_update_each_respones;
    ...
    PROCEDURE xxnss_update_final_respones (
        ...
        l_statu_update VARCHAR2(100);
        l_status       VARCHAR2(10);
    BEGIN
        l_status := 'Y';
        IF p_approver_response = 'APPROVE' THEN
            l_statu_update := 'Approved';
        ELSE
            l_statu_update := 'Rejected';
        END IF;
        ...
        p_status := l_status;
        COMMIT;
    END xxnss_update_final_respones;
